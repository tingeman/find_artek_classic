...
def multiuploader(request):
    ...
    """
    if request.method == 'POST':
        log.info(u'received POST to main multiuploader view')
        ...
        log.info(u'Got file: {0}'.format(filename))
        log.info(u'Content type: {0}'.format(file.content_type))

        #writing file manually into model
        #because we don't need form of any type.
        image = MultiuploaderImage()
        image.filename = unicode(filename)
        image.image = file
        image.key_data = image.key_generate
        image.batch_tag = request.POST['batch_tag']
        image.save()
        log.info(u'File saving done')
